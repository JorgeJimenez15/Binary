...
class Reader {
    ...
    compare(target, targetStart, targetEnd, sourceStart, sourceEnd) {        
        if (!Buffer.isBuffer(target)) throw new Error('target is not a buffer');
        if (targetStart == null) targetStart = 0;
        if (targetEnd == null) targetEnd = target.length;
        if (sourceStart == null) sourceStart = this.offset;
        if (sourceEnd == null) sourceEnd = this.buffer.length;

        return this.buffer.compare(target, targetStart, targetEnd, sourceStart, sourceEnd);
    }

    equals(otherBuffer) {
        if (!Buffer.isBuffer(otherBuffer)) throw new Error('otherBuffer is not a buffer');
